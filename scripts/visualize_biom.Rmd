...
params:
  ...
  merged_tax_tab_file: "/raw-data/khipin-data/16S/PRJNA774819_gut_bifidobacterium_SQII/work/7f/4b1d38b8176b16dfe953260cd44ffa/best_tax_merged_freq_tax.tsv"
  metadata: "/home/kpin/synology_data/data/16S/PRJNA774819_gut_bifidobacterium_SQII/metadata.tsv"
  sample_file: "/home/kpin/synology_data/data/16S/PRJNA774819_gut_bifidobacterium_SQII/samples.manifest"
  dada2_qc: "/raw-data/khipin-data/16S/PRJNA774819_gut_bifidobacterium_SQII/work/b6/4360386495bf17faa26e24e8e7ec44/dada2_qc.tsv" 
  reads_qc: "/home/kpin/synology_data/data/16S/PRJNA774819_gut_bifidobacterium_SQII/pb-16S_2022-5-31_besttax/results/reads_QC/all_samples_seqkit.readstats.tsv"
  summarised_reads_qc: "/home/kpin/synology_data/data/16S/PRJNA774819_gut_bifidobacterium_SQII/pb-16S_2022-5-31_besttax/results/reads_QC/seqkit.summarised_stats.group_by_samples.tsv"
  cutadapt_qc: "none"
  vsearch_tax_tab_file: "/raw-data/khipin-data/16S/PRJNA774819_gut_bifidobacterium_SQII/work/5f/7fbff95f62a8d82e901e1253409bdf/vsearch_merged_freq_tax.tsv"
---
...
* Total number of reads after quality filtering: `r if (skip_cutadapt != "Yes") {sum(all_read_count$input_reads)} else {sum(as.numeric(dada2_qc$input))}` (`r if (skip_cutadapt != "Yes") {paste0(round(sum(all_read_count$input_reads)/nrow(reads_qc), 2)*100, "%")} else {paste0(round(sum(as.numeric(dada2_qc$input))/nrow(reads_qc), 4), "%")}`)
...
* ASVs classified at Species level (VSEARCH with SILVA only): `r nrow(class_spec_vsearch)` (`r round(nrow(class_spec_vsearch)/total_asv, 4)*100`%)
* ASVs classified at Species level (Excluding metagenome/uncultured entries, VSEARCH with SILVA only): `r nrow(class_spec_uncultured_vsearch)` (`r round(nrow(class_spec_uncultured_vsearch)/total_asv, 4)*100`%)
* ASVs classified at Genus level (VSEARCH with SILVA only): `r nrow(class_gen_vsearch)` (`r round(nrow(class_gen_vsearch)/total_asv, 4)*100`%)
* ASVs classified at Genus level (Excluding metagenome/uncultured entries, VSEARCH with SILVA only): `r nrow(class_gen_uncultured_vsearch)` (`r round(nrow(class_gen_uncultured_vsearch)/total_asv, 4)*100`%)
