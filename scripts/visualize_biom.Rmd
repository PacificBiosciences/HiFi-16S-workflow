...
if (params$lima_qc != "none") {
  lima_qc <- read_tsv(params$lima_qc, col_types=cols())
  all_read_count <- reads_qc %>% 
    group_by(sample) %>% 
    summarise(pre_filter_reads = n()) %>% 
    inner_join(lima_qc, by="sample")
  skip_lima = "No"
  trim_lima = "Yes"
} else {
  all_read_count <- reads_qc %>% 
...
class_spec_uncultured <- tax_tab %>% filter(Species != "", !grepl("uncultured|metagenome", Species, ignore.case = TRUE))
class_gen_uncultured <- tax_tab %>% filter(Genus != "", !grepl("uncultured|metagenome", Genus, ignore.case = TRUE))
```
...
* Was primers trimmed prior to DADA2? `r paste0(" ", trim_lima)`
* Total number of reads after quality filtering: `r if (skip_lima != "Yes") {sum(all_read_count$input_reads)} else {sum(as.numeric(dada2_qc$input))}` (`r if (skip_lima != "Yes") {paste0(round(sum(all_read_count$input_reads)/nrow(reads_qc), 2)*100, "%")} else {paste0(round(sum(as.numeric(dada2_qc$input))/nrow(reads_qc), 2), "%")}`)
* Total number of reads after primers trimming (DADA2 input): `r if (skip_lima != "Yes") {sum(all_read_count$demuxed_reads)} else {paste0("Skipped lima trimming")}` (`r if (skip_lima != "Yes") {paste0(round(sum(all_read_count$demuxed_reads)/sum(all_read_count$input_reads), 2)*100, "%")} else {paste0("Skipped lima trimming")}`)
...
* Note that full length counts column will only show if lima was used to trim primers (default).
```{r, echo=FALSE}
if (skip_lima != "Yes") {
